...
1. **Coordinate lookup** - Using lat/long to look up charges (98% accuracy)
2. **Address lookup** - Using a free text address query to look up charges (98% accuracy)
3. **Eloverblik** - Using end-user MitID to get household official charges (100% accuracy)
...
| `address` | String | **Optional** | The *total* electricity price for that hour, but only spot price if charges are unavailable. |
