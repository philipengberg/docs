...
## Prices including charges (Address lookup)
The easiest way to get prices including charges is with a simple lookup.

<Warning>This approach does *not* guarantee the same degree of accuracy of charges as with [Eloverblik]().</Warning>

This process only consists of 2 steps:

<Steps>
  <Step title="Search address" icon="magnifying-glass" iconType="solid">
  Search for the address with a free text string query and let the end user pick the right address
  </Step>
...
## Prices including charges (Eloverblik)
