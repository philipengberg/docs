...
In the Min Strøm iOS app, open **Profile**, then **API-adgang** (API access). Choose **Opret API-nøgle** (Create API key). An active Min Strøm Plus subscription is required; the app offers the Plus purchase flow if needed.
...
Keep your key out of source code, URLs, screenshots, and support messages. Use your application's secret configuration for a persistent integration. Environment variables are not a substitute for protecting the machine running your integration.
...
The `data` array contains your addresses. Choose an entry whose `capabilities.consumption` is `true` and copy its `id`. This is the Consumer API address ID, not a metering-point or DAWA address ID.
...
Choose a period for which your address has data. This example requests a single UTC hour:
